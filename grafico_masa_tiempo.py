...
"""
grafico_masa_tiempo_suavizado_v45.py
════════════════════════════════════
...
de masa vs tiempo superpuesto a varias curvas de tendencia.

Novedades de la versión v45 respecto a versiones previas
────────────────────────────────────────────────────────
Novedades específicas de v45 respecto a v44
─────────────────────────────────────────────
- Se endureció la validación de la masa: se aceptan números con punto decimal
  o coma decimal, pero sólo un separador decimal en total.  No se aceptan
  separadores de miles ni formatos mixtos como ``1.222,333`` o ``1,222.333``.
- El validador del campo ``masa`` ya no aplica el rango mínimo/máximo durante
  la escritura, para no bloquear estados intermedios válidos; el rango se
  comprueba al presionar Continuar.

- Se eliminó por completo el antiguo "plan de baja y mantención de peso"
  (su curva azul, sus franjas/áreas, su casilla de visualización y todas
  las fórmulas del modelo paramétrico que lo generaban).  El programa ya
  no contiene rastro de ese modelo.
- Se añadieron CUATRO curvas de tendencia independientes, etiquetadas
  (A), (B), (C) y (D), cada una con su propia "tasa de cambio" (la
  diferencial de la tendencia, en kg/semana).  Las cuatro:
    * NO son polinomios finitos: son estimadores no paramétricos /
      kernelizados, infinitamente (o casi) diferenciables;
    * se adaptan tanto a "nubes" densas de puntos como a intervalos
      extensos sin datos, interpolando suavemente de un promedio local al
      siguiente sin inventar estructura que los datos no respalden;
    * se controlan con un único "Factor de suavidad" (slider rotulado
      "Suavidad de la curva"): a menor valor, la curva se pega más a los
      puntos; a mayor valor, se vuelve más suave.
  El detalle matemático de cada método está documentado junto a su
  función constructora en la Sección 6.
...
Uso
───
  python3 grafico_masa_tiempo_suavizado_v45.py
  python3 grafico_masa_tiempo_suavizado_v45.py mi_archivo.csv
  python3 grafico_masa_tiempo_suavizado_v45.py --help
...
MIN_MASS:   float = 30.0     # Masa mínima en kg
MAX_MASS:   float = 300.0   # Masa máxima en kg
...
class GuiStyle:
    ...
    # ── Casillas de la leyenda de visualización ──
    #    Columna izquierda: línea entre muestras + las 4 curvas de tendencia.
    #    Columna derecha:   puntos de muestra + las 4 tasas de cambio.
    #    El emoji indica el color con que se dibuja cada elemento.
    text_preview_show_point_lines: str = "Línea de muestras 🟧"
    text_preview_show_points:      str = "Muestras [kg] 🟠"
    text_preview_show_curve_a:     str = "Curva de ajuste (A) [kg] 🟥"
    text_preview_show_deriv_a:     str = "Tasa de cambio (A) [kg/sem] 🔴"
    text_preview_show_curve_b:     str = "Curva de ajuste (B) [kg] 🟩"
    text_preview_show_deriv_b:     str = "Tasa de cambio (B) [kg/sem] 🟢"
    text_preview_show_curve_c:     str = "Curva de ajuste (C) [kg] 🟦"
    text_preview_show_deriv_c:     str = "Tasa de cambio (C) [kg/sem] 🔵"
    text_preview_show_curve_d:     str = "Curva de ajuste (D) [kg] ⬛"
    text_preview_show_deriv_d:     str = "Tasa de cambio (D) [kg/sem] ⚫"
...
class PlotConfig:
    ...
    smooth_slider_min:          float = 0.05
    smooth_slider_max:          float = 2.00
    smooth_slider_resolution:   float = 0.01
    smooth_slider_tickinterval: float = 0.05

    # ── Colores ──
    #    Naranja → puntos de datos reales y la línea fina que los une.
    #    Las cuatro curvas de tendencia usan rojo / verde / azul / negro,
    #    en correspondencia con los emojis de la leyenda (🟥/🟩/🟦/⬛).
    color_points:  str = "#f28e2b"   # Naranja — puntos de datos reales
    color_curve_a: str = "#d62728"   # Rojo  — curva de tendencia (A) y su tasa de cambio
    color_curve_b: str = "#2ca02c"   # Verde — curva de tendencia (B) y su tasa de cambio
    color_curve_c: str = "#1f77b4"   # Azul  — curva de tendencia (C) y su tasa de cambio
    color_curve_d: str = "#1a1a1a"   # Negro — curva de tendencia (D) y su tasa de cambio

    # ════════════════════════════════════════════════════════════════════
    #  ESTILO COMÚN DE LAS CURVAS DE TENDENCIA
    #
    #  Las cuatro curvas (A, B, C, D) comparten el mismo aspecto: una línea
    #  central y DOS franjas (la externa ±1.0 kg, la interna ±0.5 kg) que
    #  encierran visualmente una banda de tolerancia.  Sólo cambia el color.
    # ════════════════════════════════════════════════════════════════════
    trend_curve_linewidth:    float = 1.25   # Grosor de la línea de tendencia
    trend_curve_alpha:        float = 0.85   # Opacidad de la línea
    trend_band_half_width:    float = 0.50   # Franja interna ±0.5 kg
    trend_outer_band_half_width: float = 1.00  # Franja externa ±1.0 kg
    trend_band_alpha:         float = 0.1125 # Opacidad de la franja interna
    trend_outer_band_alpha:   float = 0.075  # Opacidad de la franja externa
    trend_min_points:         int   = 3      # Mínimo de puntos para intentar un ajuste

    # ── Tasa de cambio / diferencial de cada tendencia (kg por semana) ──
    #    Se calcula numéricamente sobre la curva densa (np.gradient × 7) y se
    #    dibuja en un eje Y secundario a la derecha, en el color de su curva.
    ...
    derivative_linewidth_ratio:  float = 1.0   # Relativo a trend_curve_linewidth
    derivative_line_alpha:       float = 0.85
    derivative_line_style:       str   = ':'
    derivative_ylabel:           str   = 'Tasa de cambio [kg/sem]'
    derivative_zero_line_color:  str   = '#666666'
    derivative_zero_line_alpha:  float = 0.45
    derivative_zero_line_width:  float = 1.1
    derivative_zero_line_style:  str   = '--'

    # ── Malla densa común sobre la que se evalúan TODAS las tendencias ──
    #    Una resolución alta mantiene suave la diferencial numérica.  El costo
    #    de cómputo no es una preocupación (se prioriza la calidad).
    smooth_dense_grid_min_points: int = 1400
    smooth_dense_grid_max_points: int = 4000
    smooth_dense_grid_per_day:    int = 16

    # ── Parámetro maestro de suavidad (lo controla el slider) ──
    #
    #   > 1.0  → más suave / más redondeada / menos pegada a los puntos
    #   = 1.0  → comportamiento base
    #   < 1.0  → más fiel a los puntos / menos suavizada
    #
    # Cada método traduce este número a su propio "ancho de banda" o nivel de
    # regularización; el sentido (menor = más pegada) es idéntico en los cuatro.
    smooth_general_smoothness: float = DEFAULT_SMOOTHNESS

    # ── Parámetros de la CURVA (A): spline cúbica natural + mollifier gaussiano
    #    adaptativo.  (Es la curva que existía en versiones anteriores.) ──
    smooth_general_smoothness_visual_floor: float = 0.45
    smooth_duplicate_count_exponent: float = 0.35
    ...
    # ── Parámetros de la CURVA (B): regresión kernel de Nadaraya–Watson ──
    #    Ancho de banda global (en días) = max(piso, k·espaciado, frac·rango)·s.
    nw_bandwidth_min_days:       float = 2.0
    nw_bandwidth_max_days:       float = 90.0
    nw_bandwidth_spacing_factor: float = 1.40
    nw_bandwidth_span_fraction:  float = 0.040
    nw_kernel_radius_sigmas:     float = 6.0   # Recorte gaussiano (en σ) para eficiencia

    # ── Parámetros de la CURVA (C): regresión local lineal (LOESS) ──
    #    Ancho de banda adaptativo k-vecinos; la "fracción de span" crece con s.
    loess_span_fraction_base: float = 0.28   # Fracción de puntos en la ventana a s=1
    loess_span_fraction_min:  float = 0.06
    loess_span_fraction_max:  float = 1.00
    loess_min_window_points:  int   = 4
    loess_bandwidth_min_days: float = 1.5
    loess_bandwidth_smooth_fraction: float = 0.04  # Suaviza el perfil h(t) (C∞)

    # ── Parámetros de la CURVA (D): regresión ridge con base RBF multicuádrica ──
    #    f(t)=Σ c_j·sqrt((t-x_j)²+ε²);  ε y λ crecen con s (más suave).
    rbf_shape_spacing_factor: float = 1.10   # ε base ∝ espaciado mediano
    rbf_shape_min_days:       float = 1.5
    rbf_ridge_base:           float = 5e-3   # λ base (escala con s²)
    rbf_ridge_min:            float = 1e-8
    rbf_max_centers:          int   = 800    # Cota de centros (thinning si hay más)

    # ── Puntos de datos ──
    point_size: float = 9.6     # Tamaño del marcador (scatter `s`)
    point_alpha: float = 0.85       # Opacidad de los puntos de datos reales
    point_line_width: float = 0.60  # Grosor de la línea naranja que une las muestras
...
def _normalize_simple_decimal_string(text: str, *, allow_sign: bool) -> str:
    """Normaliza un decimal simple a formato Python, sin miles ni mezcla.

    Formato aceptado, sin signo si ``allow_sign`` es False:
        dígitos
        dígitos.dígitos
        dígitos,dígitos

    Rechaza expresamente separadores repetidos o mixtos:
        1.222.333, 1,222,333, 1.222,333, 1,222.333
    """
    s = text.strip()
    ...
    if allow_sign and s[0] in "+-":
        ...
        body = s[1:]
        if not body:
            raise ValueError(f"signo sin dígitos: {text!r}")
    else:
        sign = ""
        body = s
    ...
    separator_count = dot_count + comma_count

    if separator_count > 1:
        raise ValueError(f"más de un separador decimal: {text!r}")

    if separator_count == 0:
        if not body.isdigit():
            raise ValueError(f"formato decimal inválido: {text!r}")
        return sign + body
...
def normalize_decimal_string(text: str) -> str:
    """Normaliza una cadena numérica decimal simple a formato Python.

    Se aceptan punto o coma como separador decimal, pero sólo uno.  No se
    aceptan separadores de miles ni formatos mixtos.  Esta función permite
    signo para no romper días fraccionarios negativos en CSV antiguos.
    """
    return _normalize_simple_decimal_string(text, allow_sign=True)


def normalize_mass_decimal_string(text: str) -> str:
    """Normaliza una masa decimal simple a formato Python, sin signo."""
    return _normalize_simple_decimal_string(text, allow_sign=False)
...
def parse_mass_value(text: str) -> float:
    ...
    value = float(normalize_mass_decimal_string(text))
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║  SECCIÓN 6 — CURVAS DE TENDENCIA (cuatro métodos no paramétricos)        ║
# ║                                                                          ║
# ║  Esta sección construye, a partir de la nube de puntos (día, masa), las  ║
# ║  CUATRO curvas de tendencia que el usuario puede mostrar:                ║
# ║                                                                          ║
# ║    (A) Spline cúbica natural + mollifier gaussiano adaptativo.           ║
# ║    (B) Regresión kernel de Nadaraya–Watson (media gaussiana ponderada).  ║
# ║    (C) Regresión local lineal (LOESS) con pesos gaussianos y ancho       ║
# ║        de banda adaptativo de k-vecinos.                                 ║
# ║    (D) Regresión ridge con base de funciones radiales (RBF)              ║
# ║        multicuádricas, resuelta globalmente.                             ║
# ║                                                                          ║
# ║  Las cuatro son:                                                         ║
# ║    · NO polinómicas finitas (son estimadores kernelizados / por núcleos);║
# ║    · suaves / diferenciables (al menos C¹; A, B y D son C∞);             ║
# ║    · gobernadas por un único factor de suavidad ``s`` (el slider).       ║
# ║      Menor ``s`` ⇒ la curva se pega más a los puntos; mayor ``s`` ⇒ más  ║
# ║      suave.  Cada método traduce ``s`` a su propio ancho de banda o      ║
# ║      nivel de regularización.                                            ║
# ║                                                                          ║
# ║  Todas devuelven el mismo contrato:                                      ║
# ║      (x_grid, y_smooth, r2)  evaluadas sobre una malla densa común,      ║
# ║  o ``None`` si no hay puntos suficientes.  La tasa de cambio (kg/sem)    ║
# ║  de cualquiera se obtiene con ``build_trend_derivative_curve`` (la       ║
# ║  diferencial numérica de la curva densa, × 7).                           ║
# ╚════════════════════════════════════════════════════════════════════════════╝
...
# ────────────────────────────────────────────────────────────────────────────
#  Utilidades compartidas por las cuatro curvas de tendencia
# ────────────────────────────────────────────────────────────────────────────

def _trend_data_scale(x_unique: np.ndarray) -> tuple[float, float]:
    """Devuelve ``(span, espaciado_mediano)`` de los nodos x únicos.

    El *span* es el rango temporal cubierto por los datos y el espaciado
    mediano resume la densidad típica de muestreo.  Ambos sirven para fijar,
    de forma robusta y data-driven, el ancho de banda base de cada método.
    """
    span = float(x_unique[-1] - x_unique[0]) if x_unique.size >= 2 else 0.0
    if x_unique.size >= 2:
        spacings = np.diff(x_unique)
    ...
    if not math.isfinite(median_spacing) or median_spacing <= 0.0:
        median_spacing = max(span / max(x_unique.size, 1), 1.0)
    return span, median_spacing
...
def _prepare_trend_inputs(
    ...
    """Preprocesado común: ordena, agrega x duplicados y arma la malla densa.

    Devuelve la tupla
        ``(x_data, y_data, x_unique, y_mean, counts, x_grid, x_start, x_end)``
    o ``None`` si no hay datos suficientes.  Agrupar los x duplicados por su
    promedio evita que una "nube" con muchas medidas el mismo día sesgue los
    métodos; ``counts`` conserva la multiplicidad para ponderar.
    """
    if len(points) < config.trend_min_points:
        return None

    x_data, y_data = _sorted_point_arrays(points)
    if x_data.size < config.trend_min_points:
        return None
    ...
    return x_data, y_data, x_unique, y_mean, counts, x_grid, x_start, x_end
...
def _trend_r2(
    ...
    """Coeficiente de determinación R² del ajuste evaluado en los datos."""
    y_fit = np.interp(x_data, x_grid, y_grid)
...
# ────────────────────────────────────────────────────────────────────────────
#  CURVA (A) — Spline cúbica natural + mollifier gaussiano adaptativo
#
#  Principio
#  ─────────
#  1. Se interpola una spline cúbica natural a través de los promedios diarios
#     ``y_mean`` (curva base C² que pasa exactamente por cada nodo).
#  2. Esa curva base se "molifica": se convoluciona con un núcleo gaussiano
#     cuyo ancho h(t) varía de forma CONTINUA a lo largo del tiempo (más ancho
#     donde los datos son escasos, más angosto donde abundan).  El resultado
#     es una curva muy suave (≈ C∞) que ya no pasa por los nodos sino que los
#     promedia localmente.
#
#  No es un polinomio finito: una spline es polinómica *a trozos* y, tras la
#  convolución gaussiana, la representación deja de ser polinómica.  El factor
#  de suavidad ``s`` escala el ancho de banda h(t): menor s ⇒ menos molificado
#  ⇒ más pegada a los puntos.
# ────────────────────────────────────────────────────────────────────────────
...
def build_curve_a(
    ...
    """Construye la curva (A): spline natural + mollifier gaussiano adaptativo."""
    prep = _prepare_trend_inputs(points, config)
    if prep is None:
        return None
    x_data, y_data, x_unique, y_mean, counts, x_grid, x_start, x_end = prep
    ...
    log_info(
        'Curva (A) spline+mollifier: '
        f'h_min={float(np.min(bandwidth_profile)):.2f} días, '
        f'h_max={float(np.max(bandwidth_profile)):.2f} días, R²={r2:.4f}.'
    )
    return x_grid.astype(float), y_smooth.astype(float), float(r2)


# ────────────────────────────────────────────────────────────────────────────
#  CURVA (B) — Regresión kernel de Nadaraya–Watson (media gaussiana ponderada)
#
#  Principio
#  ─────────
#  En cada instante t la tendencia es un promedio ponderado de TODAS las
#  observaciones, con pesos gaussianos que decaen con la distancia temporal:
#
#                Σ_i K_h(t − x_i) · n_i · y_i
#        ŷ(t) = ─────────────────────────────── ,   K_h(d) = exp(−½ (d/h)²)
#                  Σ_i K_h(t − x_i) · n_i
#
#  donde n_i es la multiplicidad del día i.  Es un cociente de sumas de
#  gaussianas: infinitamente diferenciable y NO polinómico.  El ancho de banda
#  h (global) se fija de forma robusta a partir del espaciado y el rango de los
#  datos y se escala por el factor de suavidad ``s`` (menor s ⇒ h menor ⇒ más
#  pegada a los puntos).
#
#  Intervalos sin datos: si un hueco es comparable a h, el cociente transita
#  suavemente del promedio de un lado al del otro (forma sigmoidea).  Si el
#  hueco es mucho mayor que h, la curva forma una meseta en el promedio local
#  de cada extremo y una transición suave en el medio — es decir, "se aproxima
#  de un promedio al siguiente" sin inventar estructura intermedia.
# ────────────────────────────────────────────────────────────────────────────
...
def build_curve_b(
    ...
    """Construye la curva (B): regresión kernel de Nadaraya–Watson gaussiana."""
    prep = _prepare_trend_inputs(points, config)
    if prep is None:
        return None
    x_data, y_data, x_unique, y_mean, counts, x_grid, x_start, x_end = prep

    span, median_spacing = _trend_data_scale(x_unique)
    s = max(float(config.smooth_general_smoothness), 1e-6)

    # Ancho de banda global escalado por la suavidad.
    h = max(
        float(config.nw_bandwidth_min_days),
        median_spacing * float(config.nw_bandwidth_spacing_factor),
        span * float(config.nw_bandwidth_span_fraction),
    ) * s
    h = min(max(h, float(config.nw_bandwidth_min_days)),
            float(config.nw_bandwidth_max_days))

    # Media gaussiana ponderada por multiplicidad (vectorizada sobre la malla).
    u = (x_grid[:, None] - x_unique[None, :]) / h
    weights = np.exp(-0.5 * u * u) * counts[None, :]
    s0 = weights.sum(axis=1)
    s1 = (weights * y_mean[None, :]).sum(axis=1)

    y = np.empty_like(x_grid, dtype=float)
    safe = s0 > 1e-300
    y[safe] = s1[safe] / s0[safe]
    # Fuera del alcance numérico del kernel (no debería ocurrir dentro del
    # rango de datos): recurrir a la interpolación lineal de los promedios.
    if np.any(~safe):
        y[~safe] = np.interp(x_grid[~safe], x_unique, y_mean)

    r2 = _trend_r2(x_data, y_data, x_grid, y)
    log_info(f'Curva (B) Nadaraya–Watson: h={h:.2f} días, R²={r2:.4f}.')
    return x_grid.astype(float), y.astype(float), float(r2)


# ────────────────────────────────────────────────────────────────────────────
#  CURVA (C) — Regresión local lineal (LOESS) con ancho de banda de k-vecinos
#
#  Principio
#  ─────────
#  En cada instante t se ajusta por mínimos cuadrados PONDERADOS una recta
#  local  y ≈ a + b·(x − t)  usando pesos gaussianos w_i = exp(−½((x_i−t)/h(t))²)
#  (multiplicados por la multiplicidad n_i).  El valor de la tendencia es la
#  ordenada al origen local  ŷ(t) = a.
#
#  El ancho de banda h(t) es ADAPTATIVO de tipo k-vecinos: es la distancia al
#  q-ésimo punto más cercano a t.  Así, en zonas densas h(t) es pequeño (curva
#  pegada a los datos) y en huecos largos h(t) crece automáticamente, de modo
#  que la recta local "puentea" el hueco continuando la pendiente de los datos
#  de ambos lados (interpola entre promedios, no inventa oscilaciones).
#  El perfil h(t) se suaviza ligeramente para que no aparezcan quiebres por el
#  cambio discreto de vecino: la curva resultante es suave (≈ C∞ a trozos).
#
#  No es un polinomio finito: aunque cada ajuste local es lineal, el estimador
#  GLOBAL es una familia de rectas que cambia de forma continua con t (un
#  estimador no paramétrico).  La fracción de vecinos crece con ``s`` (menor s
#  ⇒ ventana más chica ⇒ más pegada a los puntos).
# ────────────────────────────────────────────────────────────────────────────
...
def build_curve_c(
    ...
    """Construye la curva (C): regresión local lineal LOESS adaptativa."""
    prep = _prepare_trend_inputs(points, config)
    if prep is None:
        return None
    x_data, y_data, x_unique, y_mean, counts, x_grid, x_start, x_end = prep
    ...
    # Tamaño de ventana (k vecinos) a partir de la fracción de span.
    span_frac = float(config.loess_span_fraction_base) * s
    span_frac = min(max(span_frac, float(config.loess_span_fraction_min)),
                    float(config.loess_span_fraction_max))
    q = int(max(int(config.loess_min_window_points), math.ceil(span_frac * n)))
    q = min(max(q, 2), n)

    # Ancho de banda adaptativo = distancia al q-ésimo nodo más cercano.
    dist = np.abs(x_grid[:, None] - x_unique[None, :])  # (G, n)
    kth = min(q - 1, n - 1)
    h_raw = np.partition(dist, kth, axis=1)[:, kth]
    h_raw = np.maximum(h_raw, float(config.loess_bandwidth_min_days))

    # Suavizado leve del perfil h(t) para eliminar quiebres por cambio de vecino.
    h_prof = h_raw
    if x_grid.size >= 3:
        dx_grid = float(np.mean(np.diff(x_grid)))
        if dx_grid > 0.0:
            sigma_samples = max(
                1.0,
                (span * float(config.loess_bandwidth_smooth_fraction)) / dx_grid,
            )
            h_prof = _apply_gaussian_post_smoothing(
                h_raw, sigma_samples=sigma_samples, radius_sigmas=3.0
            )
            h_prof = np.maximum(h_prof, float(config.loess_bandwidth_min_days))

    # Ajuste local lineal en cada punto de la malla.
    y = np.empty_like(x_grid, dtype=float)
    for i in range(x_grid.size):
        t_i = x_grid[i]
        h_i = max(float(h_prof[i]), 1e-9)
        dx = x_unique - t_i
        w = np.exp(-0.5 * (dx / h_i) ** 2) * counts
        sw = float(np.sum(w))
        if sw <= 0.0:
            y[i] = float(np.interp(t_i, x_unique, y_mean))
            continue
        swx = float(np.sum(w * dx))
        swxx = float(np.sum(w * dx * dx))
        swy = float(np.sum(w * y_mean))
        swxy = float(np.sum(w * dx * y_mean))
        det = sw * swxx - swx * swx
        if abs(det) <= 1e-12 * max(sw * swxx, 1e-12):
            # Diseño degenerado (todos los pesos en un punto): media ponderada.
            y[i] = swy / sw
        else:
            # Ordenada al origen local = valor de la recta en dx = 0 (en t_i).
            y[i] = (swy * swxx - swx * swxy) / det

    r2 = _trend_r2(x_data, y_data, x_grid, y)
    log_info(
        f'Curva (C) LOESS local-lineal: q={q}/{n} vecinos, '
        f'h_med={float(np.median(h_prof)):.2f} días, R²={r2:.4f}.'
    )
    return x_grid.astype(float), y.astype(float), float(r2)


# ────────────────────────────────────────────────────────────────────────────
#  CURVA (D) — Regresión ridge con base radial (RBF) multicuádrica
#
#  Principio
#  ─────────
#  Se modela la tendencia como una combinación lineal de funciones radiales
#  multicuádricas centradas en los días con datos:
#
#        f(t) = ȳ + Σ_j c_j · φ(t − x_j) ,   φ(r) = sqrt(r² + ε²)
#
#  Los coeficientes c_j se obtienen por mínimos cuadrados REGULARIZADOS (ridge,
#  parámetro λ), ponderando cada observación por su multiplicidad:
#
#        (Φᵀ W Φ + λ I) c = Φᵀ W (y − ȳ)
#
#  Es un estimador GLOBAL (un único sistema lineal acopla todos los puntos),
#  a diferencia de los métodos locales (B, C).  La multicuádrica es C∞ y NO
#  polinómica; como φ(r) ≈ |r| para r grande, la base puede representar
#  tendencias casi lineales por tramos, lo que produce un "puenteo" natural y
#  sensato de los huecos.  Se centra en ȳ (media ponderada) para que, al subir
#  la regularización, la curva tienda al promedio de los datos (no a cero).
#
#  El factor de suavidad ``s`` agranda tanto el ancho ε del núcleo como la
#  regularización λ: menor s ⇒ ε y λ pequeños ⇒ la curva casi interpola los
#  puntos; mayor s ⇒ curva más suave y plana.
# ────────────────────────────────────────────────────────────────────────────
...
def build_curve_d(
    ...
    """Construye la curva (D): regresión ridge con base RBF multicuádrica."""
    prep = _prepare_trend_inputs(points, config)
    if prep is None:
        return None
    x_data, y_data, x_unique, y_mean, counts, x_grid, x_start, x_end = prep
    ...
    # Centros de la base: los nodos únicos, "adelgazados" si son demasiados.
    centers = x_unique
    max_centers = int(config.rbf_max_centers)
    if x_unique.size > max_centers:
        idx = np.unique(
            np.linspace(0, x_unique.size - 1, max_centers).round().astype(int)
        )
        centers = x_unique[idx]

    # Ancho del núcleo (ε) escalado por la suavidad.
    eps = max(
        float(config.rbf_shape_min_days),
        median_spacing * float(config.rbf_shape_spacing_factor),
    ) * s

    def _multiquadric(a_col: np.ndarray, b_row: np.ndarray) -> np.ndarray:
        d = a_col - b_row
        return np.sqrt(d * d + eps * eps)

    # Matriz de diseño (observaciones × centros) y pesos por multiplicidad.
    phi = _multiquadric(x_unique[:, None], centers[None, :])  # (n_obs, k)
    w = counts.astype(float)
    y_bar = float(np.sum(w * y_mean) / np.sum(w))
    residual = y_mean - y_bar

    wphi = phi * w[:, None]
    gram = phi.T @ wphi                 # ΦᵀWΦ  (k, k), simétrica ≥ 0
    rhs = wphi.T @ residual             # ΦᵀW(y−ȳ)

    k = gram.shape[0]
    diag_scale = float(np.trace(gram) / max(k, 1))
    if not math.isfinite(diag_scale) or diag_scale <= 0.0:
        diag_scale = 1.0
    lam = max(float(config.rbf_ridge_min),
              float(config.rbf_ridge_base) * s * s) * diag_scale
    gram_reg = gram + lam * np.eye(k)

    try:
        coeffs = np.linalg.solve(gram_reg, rhs)
    except np.linalg.LinAlgError:
        coeffs, *_ = np.linalg.lstsq(gram_reg, rhs, rcond=None)

    phi_eval = _multiquadric(x_grid[:, None], centers[None, :])  # (G, k)
    y = y_bar + phi_eval @ coeffs

    if not np.all(np.isfinite(y)):
        # Salvaguarda numérica: interpolación lineal de los promedios.
        log_warn('Curva (D): resultado no finito; se usa interpolación lineal.')
        y = np.interp(x_grid, x_unique, y_mean)

    r2 = _trend_r2(x_data, y_data, x_grid, y)
    log_info(
        f'Curva (D) RBF multicuádrica: ε={eps:.2f} días, λ={lam:.3e}, '
        f'centros={k}, R²={r2:.4f}.'
    )
    return x_grid.astype(float), y.astype(float), float(r2)
...
# ── Despacho por método ──────────────────────────────────────────────────────
#  Cada clave ('A'..'D') mapea a su función constructora.  Todas comparten el
#  contrato (x_grid, y, r2) | None, de modo que el renderizador las trata de
#  forma uniforme.
TREND_BUILDERS = {
    'A': build_curve_a,
    'B': build_curve_b,
...
def build_trend_curve(
    ...
    """Construye la curva de tendencia del método indicado ('A'..'D')."""
    builder = TREND_BUILDERS.get(method)
    if builder is None:
        raise ValueError(f'Método de tendencia desconocido: {method!r}')
    return builder(points, config)


def build_trend_derivative_curve(
    x_smooth: np.ndarray,
    y_smooth: np.ndarray,
    config: PlotConfig,
) -> tuple[np.ndarray, np.ndarray] | None:
    """Tasa de cambio (diferencial) de una curva de tendencia, en kg/semana.

    Se remuestrea la curva cada ``derivative_sample_step_days`` días y se
    deriva numéricamente con ``np.gradient`` (diferencias centradas).  Como las
    cuatro curvas son suaves, la diferencial numérica es estable.  El resultado
    se expresa en kg/semana multiplicando la pendiente diaria por 7.
    """
...
class PlotRenderOptions:
    ...
    show_point_lines: bool = False   # Línea naranja fina uniendo las muestras
    show_points:      bool = True    # Puntos de datos reales (naranja)
    show_curve_a:     bool = True    # Curva de tendencia (A) — rojo
    show_deriv_a:     bool = True    # Tasa de cambio de (A) — rojo
    show_curve_b:     bool = False   # Curva de tendencia (B) — verde
    show_deriv_b:     bool = False   # Tasa de cambio de (B) — verde
    show_curve_c:     bool = False   # Curva de tendencia (C) — azul
    show_deriv_c:     bool = False   # Tasa de cambio de (C) — azul
    show_curve_d:     bool = False   # Curva de tendencia (D) — negro
    show_deriv_d:     bool = False   # Tasa de cambio de (D) — negro
...
# Orden de las cuatro curvas de tendencia y los atributos de RenderOptions /
# PlotConfig asociados a cada una.  Permite recorrerlas con un único bucle.
_TREND_PLAN = (
    # (método, attr_curva, attr_derivada, attr_color)
    ('A', 'show_curve_a', 'show_deriv_a', 'color_curve_a'),
    ('B', 'show_curve_b', 'show_deriv_b', 'color_curve_b'),
    ('C', 'show_curve_c', 'show_deriv_c', 'color_curve_c'),
    ('D', 'show_curve_d', 'show_deriv_d', 'color_curve_d'),
)


def _draw_trend_curve(
    ax,
    x_grid: np.ndarray,
    y_curve: np.ndarray,
    color: str,
    ...
    """Dibuja una curva de tendencia: franja externa, franja interna y línea.

    Las cuatro curvas (A, B, C, D) comparten exactamente este aspecto; sólo
    cambia el color.  Es el mismo estilo "línea + franjas" que tenía la curva
    de ajuste original.
    """
    ax.fill_between(
        x_grid,
        y_curve - config.trend_outer_band_half_width,
        y_curve + config.trend_outer_band_half_width,
        color=color,
        alpha=config.trend_outer_band_alpha,
        linewidth=0.0,
        edgecolor='none',
        zorder=base_zorder,
    )
    ax.fill_between(
        x_grid,
        y_curve - config.trend_band_half_width,
        y_curve + config.trend_band_half_width,
        color=color,
        alpha=config.trend_band_alpha,
        linewidth=0.0,
        edgecolor='none',
        zorder=base_zorder + 0.05,
    )
    ax.plot(
        x_grid,
        y_curve,
        color=color,
        alpha=config.trend_curve_alpha,
        linewidth=config.trend_curve_linewidth,
        zorder=base_zorder + 0.10,
    )
...
def render_plot(
    ...
    # ── Calcular (una sola vez) cada curva de tendencia que se necesite ──
    #    Una curva se calcula si su línea O su tasa de cambio están activas.
    trend_cache: dict[str, tuple[np.ndarray, np.ndarray, float] | None] = {}
    for method, attr_curve, attr_deriv, _attr_color in _TREND_PLAN:
        if getattr(render_options, attr_curve) or getattr(render_options, attr_deriv):
            result = build_trend_curve(method, ordered_points, config)
            trend_cache[method] = result
            if result is None and ordered_points:
                log_info(
                    f'Curva ({method}) omitida: puntos insuficientes para el ajuste.'
                )
    ...
    if render_options.show_point_lines and len(visible) >= 2:
        ax.plot(
            ...
            zorder=base_zorder + 0.90,
        )

    # ── Curvas de tendencia A/B/C/D (línea + franjas) ──
    #    Se apilan en zorder en el orden A, B, C, D.
    for index, (method, attr_curve, _attr_deriv, attr_color) in enumerate(_TREND_PLAN):
        if not getattr(render_options, attr_curve):
            continue
        curve = trend_cache.get(method)
        if curve is None:
            continue
        x_grid, y_curve, _r2 = curve
        _draw_trend_curve(
            ax,
            x_grid,
            y_curve,
            getattr(config, attr_color),
            config,
            base_zorder=base_zorder + 0.20 + index * 0.20,
        )

    # ── Tasas de cambio (diferenciales) en un único eje derecho (kg/sem) ──
    any_derivative = any(
        getattr(render_options, attr_deriv) and trend_cache.get(method) is not None
        for method, _attr_curve, attr_deriv, _attr_color in _TREND_PLAN
    )
    if any_derivative:
        ax_right = ax.twinx()
        ...
        # Una sola línea horizontal neutra en y=0 (gris), común a todas.
        ax_right.hlines(
            y=0.0,
            xmin=config.x_min, xmax=config.x_max,
            colors=config.derivative_zero_line_color,
            ...
            zorder=base_zorder + 1.05,
        )

        for index, (method, _attr_curve, attr_deriv, attr_color) in enumerate(_TREND_PLAN):
            if not getattr(render_options, attr_deriv):
                continue
            curve = trend_cache.get(method)
            if curve is None:
                continue
            x_grid, y_curve, _r2 = curve
            derivative_curve = build_trend_derivative_curve(x_grid, y_curve, config)
            if derivative_curve is None:
                continue
            x_derivative, y_derivative = derivative_curve
            ax_right.plot(
                x_derivative,
                y_derivative,
                color=getattr(config, attr_color),
                alpha=config.derivative_line_alpha,
                linewidth=(config.trend_curve_linewidth * config.derivative_linewidth_ratio),
                linestyle=config.derivative_line_style,
                zorder=base_zorder + 1.10 + index * 0.05,
            )
...
class _MassFieldValidator(QValidator):
    """Validador del campo de masa — acepta punto o coma decimal, sólo uno.

    El validador sólo controla la forma del texto mientras se escribe.  El
    rango real ``MIN_MASS``/``MAX_MASS`` se comprueba al confirmar, para no
    bloquear prefijos intermedios como ``9`` al escribir ``92,5``.
    """

    @staticmethod
    def _is_partial_decimal_text(text: str) -> bool:
        if text == '':
            return True
        ...
        for ch in text:
            if ch.isdigit():
                continue
            if ch in '.,' and not seen_separator:
                seen_separator = True
                continue
            return False
        return True
    ...
    @staticmethod
    def _is_complete_decimal_text(text: str) -> bool:
    ...
    def validate(self, text: str, pos: int):  # type: ignore[override]
        if not self._is_partial_decimal_text(text):
            return (QValidator.State.Invalid, text, pos)

        if self._is_complete_decimal_text(text):
            return (QValidator.State.Acceptable, text, pos)

        # Permite estados editables como '', '.', ',', '92.' o '92,'.
        # No son valores finales válidos, pero sí pueden convertirse en uno.
...
class MassInputApp(QWidget):
    ...
    def _relock_window_to_current_page(self) -> None:
        ...
        Corrige el bug donde el borde derecho e inferior de la vista previa
        quedaban recortados: tras cambiar el pixmap, ``adjustSize()`` por sí
        solo medía el layout antes de que el nuevo ``sizeHint`` del label se
        propagara hacia arriba, dejando la ventana unos píxeles más pequeña
        que su contenido.  La solución es:

          1. Invalidar y reactivar explícitamente el layout para forzar el
             recálculo de los ``sizeHint`` de todos los hijos (incluido el
             label de la imagen, cuyo tamaño fijo ya es el del pixmap).
          2. Fijar el tamaño al MÁXIMO entre el tamaño actual y el ``sizeHint``
             recién calculado, más un pequeño margen de seguridad, de modo que
             nunca quede por debajo de lo que el contenido necesita.
        """
        # Liberamos el bloqueo anterior para poder recalcular el tamaño.
        self.setMinimumSize(0, 0)
        self.setMaximumSize(16777215, 16777215)

        # Forzamos a Qt a recomputar los sizeHint de toda la jerarquía.
        layout = self.layout()
        ...
        # Tomamos el máximo entre lo que reporta adjustSize() y el sizeHint
        # del layout, y añadimos un pequeño colchón para evitar el recorte de
        # los bordes derecho/inferior por redondeos de submuestreo del pixmap.
        hint = self.sizeHint()
        current = self.size()
        safety_pad = 2  # px — evita el recorte de 1 px por redondeo
        target_w = max(current.width(), hint.width()) + safety_pad
        target_h = max(current.height(), hint.height()) + safety_pad
    ...
    def _build_preview_page(self, parent: QWidget) -> None:
        ...
        # Diez checkboxes dispuestos en una grilla 5×2 que reproduce la leyenda:
        #
        #   [ ] Línea de muestras 🟠        [X] Muestras [kg] 🟧
        #   [X] Curva de ajuste (A) 🟥      [X] Tasa de cambio (A) 🔴
        #   [ ] Curva de ajuste (B) 🟩      [ ] Tasa de cambio (B) 🟢
        #   [ ] Curva de ajuste (C) 🟦      [ ] Tasa de cambio (C) 🔵
        #   [ ] Curva de ajuste (D) ⬛      [ ] Tasa de cambio (D) ⚫
        #
        # Columna izquierda = líneas/curvas; columna derecha = muestras/tasas.
        self._chk_point_lines = QCheckBox(s.text_preview_show_point_lines, options_group)
        self._chk_points      = QCheckBox(s.text_preview_show_points, options_group)
        self._chk_curve_a     = QCheckBox(s.text_preview_show_curve_a, options_group)
        self._chk_deriv_a     = QCheckBox(s.text_preview_show_deriv_a, options_group)
        self._chk_curve_b     = QCheckBox(s.text_preview_show_curve_b, options_group)
        self._chk_deriv_b     = QCheckBox(s.text_preview_show_deriv_b, options_group)
        self._chk_curve_c     = QCheckBox(s.text_preview_show_curve_c, options_group)
        self._chk_deriv_c     = QCheckBox(s.text_preview_show_deriv_c, options_group)
        self._chk_curve_d     = QCheckBox(s.text_preview_show_curve_d, options_group)
        self._chk_deriv_d     = QCheckBox(s.text_preview_show_deriv_d, options_group)

        # Estado inicial (valores por defecto marcados con X en la leyenda):
        #   Muestras, Curva de ajuste (A) y Tasa de cambio (A).
        self._chk_point_lines.setChecked(False)
        self._chk_points.setChecked(True)
        self._chk_curve_a.setChecked(True)
        self._chk_deriv_a.setChecked(True)
        self._chk_curve_b.setChecked(False)
        self._chk_deriv_b.setChecked(False)
        self._chk_curve_c.setChecked(False)
        self._chk_deriv_c.setChecked(False)
        self._chk_curve_d.setChecked(False)
        self._chk_deriv_d.setChecked(False)

        # Todos los toggles re-renderizan; las curvas/tasas además recalculan
        # el estado del slider (que sólo aplica si hay alguna tendencia activa).
        ...
        for _chk in (
            self._chk_curve_a, self._chk_deriv_a,
            self._chk_curve_b, self._chk_deriv_b,
            self._chk_curve_c, self._chk_deriv_c,
            self._chk_curve_d, self._chk_deriv_d,
        ):
            _chk.toggled.connect(lambda _c: self._on_toggle_adjustment())

        options_grid.addWidget(self._chk_point_lines, 0, 0)
        options_grid.addWidget(self._chk_points,      0, 1)
        options_grid.addWidget(self._chk_curve_a,     1, 0)
        options_grid.addWidget(self._chk_deriv_a,     1, 1)
        options_grid.addWidget(self._chk_curve_b,     2, 0)
        options_grid.addWidget(self._chk_deriv_b,     2, 1)
        options_grid.addWidget(self._chk_curve_c,     3, 0)
        options_grid.addWidget(self._chk_deriv_c,     3, 1)
        options_grid.addWidget(self._chk_curve_d,     4, 0)
        options_grid.addWidget(self._chk_deriv_d,     4, 1)
        ...
        self._smoothness_scale.setMinimumWidth(max(200, int(self.config.preview_width_px) - 60))
        ...
        self._interactive_widgets.extend([
            ...
            self._chk_point_lines, self._chk_points,
            self._chk_curve_a, self._chk_deriv_a,
            self._chk_curve_b, self._chk_deriv_b,
            self._chk_curve_c, self._chk_deriv_c,
            self._chk_curve_d, self._chk_deriv_d,
    ...
    def _parse_all_inputs(self) -> tuple[datetime, float] | None:
        ...
        if not raw_mass:
            ...
            return None
        try:
            mass_val = parse_mass_value(raw_mass)
        except (ValueError, TypeError):
    ...
    def _build_render_options(self) -> PlotRenderOptions:
        ...
        return PlotRenderOptions(
            ...
            show_curve_a=bool(self._chk_curve_a.isChecked()),
            show_deriv_a=bool(self._chk_deriv_a.isChecked()),
            show_curve_b=bool(self._chk_curve_b.isChecked()),
            show_deriv_b=bool(self._chk_deriv_b.isChecked()),
            show_curve_c=bool(self._chk_curve_c.isChecked()),
            show_deriv_c=bool(self._chk_deriv_c.isChecked()),
            show_curve_d=bool(self._chk_curve_d.isChecked()),
            show_deriv_d=bool(self._chk_deriv_d.isChecked()),
        )
    ...
    def _trend_checkboxes(self) -> tuple[QCheckBox, ...]:
        """Checkboxes de curvas/tasas que dependen del slider de suavidad."""
        return (
            self._chk_curve_a, self._chk_deriv_a,
            self._chk_curve_b, self._chk_deriv_b,
            self._chk_curve_c, self._chk_deriv_c,
    ...
    def _any_trend_active(self) -> bool:
        """¿Hay alguna curva de tendencia o tasa de cambio activa?"""
        return any(chk.isChecked() for chk in self._trend_checkboxes())
    ...
    def _apply_preview_default_options(self, *, rerender: bool) -> None:
        ...
        try:
            self._chk_point_lines.setChecked(False)
            self._chk_points.setChecked(True)
            self._chk_curve_a.setChecked(False)
            self._chk_deriv_a.setChecked(False)
            self._chk_curve_b.setChecked(True)
            self._chk_deriv_b.setChecked(True)
            self._chk_curve_c.setChecked(False)
            self._chk_deriv_c.setChecked(False)
            self._chk_curve_d.setChecked(False)
            self._chk_deriv_d.setChecked(False)
    ...
    def _update_adjustment_controls_state(self) -> None:
        """Habilita o deshabilita el slider según haya tendencias activas."""
        enabled = self._any_trend_active()
    ...
    def _build_effective_plot_config(self, *, preview: bool) -> PlotConfig:
        ...
        if not preview:
            return effective
        preview_width = max(1, int(effective.preview_width_px))
        ...
        return replace(effective, width_px=preview_width, height_px=preview_height, dpi=preview_dpi)
    ...
    def _load_preview_into_qt(self, preview_path: Path) -> None:
        ...
        # Fuerza al label a adoptar el tamaño real de la imagen para que el
        # frame que lo contiene crezca correctamente.
        self._preview_image_label.setFixedSize(pixmap.size())
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            "Suavidad global de las curvas de tendencia: >1 más suave, "
